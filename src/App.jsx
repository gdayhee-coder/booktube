...
export default function App() {
  ...
  const search = async (query, codeOverride, mode) => {
    ...
    try {
      const r = await fetch("/api/youtube?q=" + encodeURIComponent(term) + (m === "listen" ? "&mode=listen" : ""));
      const j = await r.json();
      if (j.error) throw new Error(j.error);
      setResults(j.items || []);
  ...
  if (!user) {
    return (
      <div style={{ fontFamily:FF, background:C.bg, minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:20 }}>
        ...
        <div style={{ fontSize:24, fontWeight:900, color:C.text, letterSpacing:"-0.5px" }}>북튜브</div>
        <div style={{ fontSize:12.5, color:C.textMuted, marginTop:4, marginBottom:24, textAlign:"center", lineHeight:1.6 }}>원서 제목을 검색하면 낭독 영상을 찾아줘요<br/>로그인 후 이용할 수 있어요</div>
        <div style={{ width:"100%", maxWidth:320, background:C.paper, borderRadius:16, padding:22, border:`1.5px solid ${C.border}`, boxShadow:"0 4px 16px rgba(94,112,73,0.08)" }}>
          ...
          <button onClick={doLogin}
            style={{ width:"100%", padding:"13px", background:C.woodDark, color:"#fff", border:"none", borderRadius:10, fontSize:15, fontWeight:800, cursor:"pointer", fontFamily:FF, display:"flex", alignItems:"center", justifyContent:"center", gap:7 }}>
            {Ic.key("#fff", 15)} 로그인
          </button>
          {loginErr ? (
            <div style={{ marginTop:10, textAlign:"center" }}>
              <div style={{ fontSize:12, color:C.warn, marginBottom:8 }}>{loginErr}</div>
            ...
            <div style={{ fontSize:11, color:C.textMuted, marginTop:12, textAlign:"center", lineHeight:1.6 }}>선생님께 받은 아이디와 비밀번호로<br/>로그인하세요 🌿</div>
          )}
        </div>
      </div>
    );
