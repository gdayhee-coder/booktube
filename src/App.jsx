...
export default function App() {
  ...
  const search = async (query, codeOverride) => {
    ...
    try {
      ...
      const r = await fetch("/api/youtube?q=" + encodeURIComponent(term) + (ac ? "&code=" + encodeURIComponent(ac) : ""));
      if (r.status === 401) {
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(37,43,35,0.85)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div style={{ background:C.paper, borderRadius:18, padding:"28px 24px", maxWidth:340, width:"100%", textAlign:"center", boxShadow:"0 10px 40px rgba(0,0,0,0.3)" }}>
            ...
            <input value={codeIn} onChange={(e) => setCodeIn(e.target.value)} type="password"
              onKeyDown={(e) => { if (e.key === "Enter" && codeIn.trim()) { const c = codeIn.trim(); localStorage.setItem("bt-code", c); setLocked(false); setCodeIn(""); if (pendingQ) search(pendingQ, c); } }}
              placeholder="접속 코드"
            ...
            <button onClick={() => { if (!codeIn.trim()) return; const c = codeIn.trim(); localStorage.setItem("bt-code", c); setLocked(false); setCodeIn(""); if (pendingQ) search(pendingQ, c); }}
